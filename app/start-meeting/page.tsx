...
import { Loader2, Video, Sparkles, ShieldCheck } from "lucide-react"
...
export default function StartMeetingPage() {
  ...
  JSON.stringify(data, null, 2)
   ...
   // 👇 HANDLE "BOT ALREADY ACTIVE" CASE
if (!res.ok && data.detail?.includes("Bot already active")) {
  console.warn("Bot stuck active, ending old meeting…")

  // 1️⃣ END OLD BOT
  await fetch("/api/end-meeting", { method: "POST" })

  // 2️⃣ RETRY START
  const retryRes = await fetch("/api/start-meeting", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
  })
  ...
  if (!retryRes.ok) {
    setError("Failed to restart meeting")
    return
  }
...
// 👇 NORMAL ERROR (NOT bot-active)
if (!res.ok) {
  setError(data.message || "Bot failed to join meeting")
  return
}
